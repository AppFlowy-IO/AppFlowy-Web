...
import { DatabaseGridSelectors, DatabaseListSelectors, DatabaseViewSelectors } from '../../support/selectors';
...
async function openListGroupSettings(page: Page) {
  ...
  await page.getByTestId('list-group-settings-trigger').click();
...
When('I hide List group {string} from group settings', async ({ page }, groupName: string) => {
  await openListGroupSettings(page);
  const groupRow = page.locator('[data-testid^="list-group-visibility-"]').filter({ hasText: groupName });
