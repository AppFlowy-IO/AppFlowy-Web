...
Feature: Server publish settings are preserved across republish
  New publications use the server's default with comments enabled.
  Unpublishing and republishing must keep the publisher's last comments setting.
  Only the server stores the setting so the same value is used in every browser.

  Background:
    Given a blank document page is open

  Scenario: Republishing preserves the server default when comments were never changed
    When I type "Comments default doc" in the editor
  ...
  Scenario: Returning to a published tab refreshes comment visibility from the server
    ...
    And I return to the published page
