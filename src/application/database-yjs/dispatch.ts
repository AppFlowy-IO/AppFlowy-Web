...
export function useUpdateSelectOptions (fieldId: string) {
  const database = useDatabase();
  const sharedRoot = useSharedRoot();
  const field = database.get(YjsDatabaseKey.fields)?.get(fieldId);
  ...
  return useCallback((options: SelectOption[]) => {
    executeOperations(sharedRoot, [() => {
      const fieldType = Number(field.get(YjsDatabaseKey.type));
      ...
      const selectOptions = JSON.parse(content) as SelectTypeOption;

      selectOptions.options = options;

      typeOption.set(YjsDatabaseKey.content, JSON.stringify(selectOptions));
    }], 'updateSelectOptions');
  }, [field, sharedRoot]);
}
