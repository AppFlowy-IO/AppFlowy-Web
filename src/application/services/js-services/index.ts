...
export class AFClientService implements AFService {
  ...
  async getPublishInfo (viewId: string) {
    ...
    const data = {
      ...
      commentEnabled: info.comment_enabled,
  ...
  async updatePublishConfig (viewId: string, config: UpdatePublishConfigPayload) {
    return APIService.updatePublishConfig(viewId, config);
  }
