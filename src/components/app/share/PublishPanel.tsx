...
function PublishPanel ({ viewId, opened, onClose }: { viewId: string; onClose: () => void; opened: boolean }) {
  ...
  const [commentEnabled, setCommentEnabled] = React.useState<boolean>(false);
  const [duplicateEnabled, setDuplicateEnabled] = React.useState<boolean>(false);
  ...
  useEffect(() => {
    if (opened && publishInfo) {
      setCommentEnabled(publishInfo.commentEnabled || false);
      setDuplicateEnabled(publishInfo.duplicateEnabled || false);
    }
  }, [opened, publishInfo]);
        ...
        <div className={'p-1.5 text-sm flex items-center gap-4 justify-between'}>
          ...
          <Switch
            ...
            onChange={e => {
              ...
              void updatePublishConfig({ comment_enabled: e.target.checked });
            }}
            size={'small'}
          />
        </div>
        <div className={'p-1.5  text-sm flex items-center gap-4 justify-between'}>
          ...
          <Switch
            ...
            onChange={e => {
              ...
              void updatePublishConfig({ duplicate_enabled: e.target.checked });
            }}
            size={'small'}
          />
  ...
  }, [commentEnabled, duplicateEnabled, handlePublish, handleUnpublish, isOwner, isPublisher, onClose, publishInfo, t, unpublishLoading, url, updatePublishConfig, view]);
