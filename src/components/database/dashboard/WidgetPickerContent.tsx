import { KeyboardEvent, memo, useCallback, useDeferredValue, useMemo, useRef, useState } from 'react';
...
interface PickerDatabaseListProps {
  databases: WidgetPickerDatabase[];
...
/** The "New view" database choice; memoized for the same reason as the group list. */
const PickerDatabaseList = memo(function PickerDatabaseList({
  databases,
  selectedDatabaseId,
  creating,
  loading,
  onSelect,
}: PickerDatabaseListProps) {
  const { t } = useTranslation();

  return (
    <div className='flex max-h-[160px] flex-col overflow-y-auto pr-1' role='radiogroup'>
      {databases.map((entry) => {
        const selected = entry.databaseId === selectedDatabaseId;
        ...
        return (
          <button
            ...
            onClick={() => onSelect(entry.databaseId)}
            role='radio'
            type='button'
          >
            <ViewIcon layout={ViewLayout.Grid} size='small' />
            <span className='min-w-0 flex-1 truncate'>
              {entry.isHost ? t('dashboard.picker.thisDatabase', { defaultValue: 'This database' }) : entry.name || t('untitled')}
            </span>
...
/**
 * The picker's dialog content: an existing view of any database, or a new
 * view created on the spot. Loaded on demand by `WidgetPicker`; a dashboard
 * that is only viewed never needs it.
 */
export function WidgetPickerContent({ request, onPick, createView, canCreateInOtherDatabases }: WidgetPickerContentProps) {
  const { t } = useTranslation();
  const hostContext = useDatabaseContext();
  const database = useDatabase();
  const { hostDatabaseId, dashboardViewId } = useDashboardContext();
  const { hostViewIds, rows } = useDashboardLayout();
  ...
  const handleCreate = async (layout: DatabaseViewLayout) => {
    ...
    try {
      ...
      // Also after an unmount: React 18 ignores the update, and a StrictMode
      // effect replay must not leave the dialog locked in its creating state.
      setCreatingLayout(null);
    }
  };
