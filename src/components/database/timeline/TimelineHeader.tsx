...
/**
 * The column (or segment) labels alone. Memoized apart from the drag
 * highlight, which changes on every drag step while the labels only change
 * with the scroll window or the scale.
 */
const HeaderColumns = memo(
  ({ mode, segments, columns, stickyOffset }: Omit<TimelineHeaderProps, 'canvasWidth' | 'highlight'>) => (
    <>
      {mode === 'segments'
...
export const TimelineHeader = memo(
  ({ mode, segments, columns, canvasWidth, highlight, stickyOffset }: TimelineHeaderProps) => {
    return (
      ...
      >
        <HeaderColumns mode={mode} segments={segments} columns={columns} stickyOffset={stickyOffset} />
        {highlight ? (
          <div
            aria-hidden
